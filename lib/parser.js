class Parser {
  ...
  processShort(arg) {
    for (let index = 1; index < arg.length; index++) {
      ...
      if (this.settings[target].type === Boolean) {
        this.result[target] = !this.result[target];
        this.target = this.catchAll;
      } else {
  ...
  processLong(arg) {
    const assignmentPoint = arg.indexOf('=');
    if (assignmentPoint === 0) {
    ...
    if (assignmentPoint === -1) {
      ...
      if (!this.settings[target]) {
        this.errors.push(`unknown option '${target}'`);
        return;
      }
      this.assign(target, arg.slice(assignmentPoint + 1));
    }
  }
  parse(commandArgs) {
    this.errors = [];
